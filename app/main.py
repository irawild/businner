...
#  Articles API
@app.get("/articles/")
def get_articles_list(type = None, current_user: User = Depends(get_current_active_user)):
    if type is None:
...
@app.post("/articles/")
def post_article(article_received: Article, current_user: User = Depends(get_current_active_user)):
    return article.post(article_received)

@app.get("/articles/{id}", response_model=Article)
def get_article(id: int = 0, current_user: User = Depends(get_current_active_user)):
    return article.get(id)

@app.delete("/articles/{id}", response_model=Article)
def delete_article(id: int = 0, current_user: User = Depends(get_current_active_user)):
    return article.delete(id)

@app.put("/articles/{id}", response_model=Article)
def put_article(id: int = 0, article_received: Article = None, current_user: User = Depends(get_current_active_user)):
